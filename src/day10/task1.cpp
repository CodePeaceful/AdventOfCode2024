...
int calculateRoutes(const std::vector<std::vector<int>>& map, int i, int j) {
    ...
    std::set<std::pair<int, int>> thisStep;
    std::set<std::pair<int, int>> nextStep;
    thisStep.insert(std::pair(i, j));
    for (int k = 1; k < 10; ++k) {
        for (auto [top, left] : thisStep) {
            if (top - 1 >= 0 && map[top - 1][left] == k) {
                nextStep.insert(std::pair(top - 1, left));
            }
            if (top + 1 < ilimit && map[top + 1][left] == k) {
                nextStep.insert(std::pair(top + 1, left));
            }
            if (left - 1 >= 0 && map[top][left - 1] == k) {
                nextStep.insert(std::pair(top, left - 1));
            }
